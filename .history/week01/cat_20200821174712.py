...
def get_url_name(movie_url):
    ...
    bs_info = bs(response.text, 'html.parser')for tags in bs_info.find_all('div', attrs={'class': 'movie-hover-info'}):
        print(tags)
        print(tags.find('span',).text)
        for atag in tags.find_all('div', attrs={'class': 'movie-hover-title'}):

            # print(atag.find('div', attrs={'class': 'movie-hover-title'}).text)
            print("hah")
