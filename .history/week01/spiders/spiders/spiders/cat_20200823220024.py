...
class CatSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for movie in movies:
            ...
            span_list = movie.xpath('./div[@class="movie-hover-title"]')
            # print(span_name_list)

            # for span_node in span_list:
            #     # print(span_node)
            #     span_node_parent = span_node.xpath('../text()')
            ...
            # #     print(span_name)
            #     if(span_name == "类型:"):
            #         movie_type = span_node_parent[1].extract().replace("\n", "").strip()
            #         print(movie_type)
            #     # elif(span_name == "上映时间:"):
            #     #     movie_plan_time = span_name_1.xpath('../div[@class="movie-hover-title"]/text()')
            #     #     print(movie_plan_time)
            #     elif(span_name == "主演:"):
            #         movie_performer = span_node_parent[1].extract().replace("\n", "").strip()
