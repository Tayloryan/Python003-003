...
class CatSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for movie in movies:
            ...
            movie_plan_time = ""
            movie_performer = ""
            span_name_list = movie.xpath('./div[@class="movie-hover-title"]/span[1]//text()')
            ...
            for span_name_1 in span_name_list:
                ...
                print(span_name)
                if(span_name == "类型:"):
                    movie_type = span_name_1.xpath('../div[@class="movie-hover-title"]/text()').extract().strip()
                    print(movie_type)
                elif(span_name == "上映时间:"):
                    movie_plan_time = span_name_1.xpath('../div[@class="movie-hover-title"]/text()').extract().strip()
                    print(movie_plan_time)
                elif(span_name == "主演:"):
                    movie_performer = span_name_1.xpath('./div[@class="movie-hover-title"]/text()').extract().strip()
